...
document
  ...
  .addEventListener("click", newBookPopUp);
document.querySelector(".closeButton").addEventListener("click", closePopUp);
document
  .querySelector(".submitButton")
  .addEventListener("click", handleFormInput);
...
function createBookCard(myBook) {
  ...
  for (prop in myBook) {
    ...
    if (prop === "title" || prop === "author" || prop === "numOfPages") {
      ...
      document
        .querySelector(`[data-array="${myLibrary.indexOf(myBook)}"]`)
    ...
    if (prop === "haveRead") {
      ...
      myButtonNode.className =
        myBook[prop] === "on" ? "read-book" : "not-read-book";
...
function toggleRead(e) {
    ...
    e.target.textContent =
      e.target.className === "read-book" ? "X Read" : "✓ Read";
    e.target.className =
    ...
    myLibrary[bookIndex].haveRead =
      myLibrary[bookIndex].haveRead === "on" ? "off" : "on";
  }
  ...
  function handleFormInput(e) {
    ...
    if (
      ...
      !myForm.numberOfPages.validity.valueMissing
    ) {
      addBookToLibrary(
        myForm.title.value,
        myForm.author.value,
